...
_QCHV_HINTS = (
    ...
    "ke hoach hoc tap chuan toan khoa",
    "khht chuẩn toàn khóa",
    "khht chuan toan khoa",
)
...
def _normalize_domain_value(domain: str) -> str:
    ...
    if value in {"QCHV", "QUY_CHE", "QUY CHE", "QUYCHE"}:
        return "QCHV"
    if value in {"THONG_BAO", "THONG BAO", "THÔNG_BÁO", "THÔNG BÁO"}:
        return "THONG_BAO"
...
"""
    ...
    async def _generate_thong_bao_answer(
        self,
        query: str,
        context: str,
    ) -> str:
...
"""
        return (await call_model_9b(prompt, temperature=0.2)).strip()
